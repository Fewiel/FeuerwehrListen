...
public class Migration_019_AddMemberUnitsTable : Migration
{
    ...
    {
        // SQLite unterstützt kein ALTER TABLE ADD CONSTRAINT — der zusammengesetzte
        // Primärschlüssel wird per .PrimaryKey() pro Spalte direkt beim CREATE TABLE gesetzt.
        // Idempotent: nur anlegen, wenn die Tabelle nicht bereits existiert (z. B. aus
        // einem vorherigen, gescheiterten Migrations-Versuch).
        if (!Schema.Table("MemberUnit").Exists())
        {
            Create.Table("MemberUnit")
                .WithColumn("MemberId").AsInt32().NotNullable().PrimaryKey()
                .WithColumn("UnitNumber").AsInt32().NotNullable().PrimaryKey();
